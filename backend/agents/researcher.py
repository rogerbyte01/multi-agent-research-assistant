...
def perform_research(sub_questions: List[str]) -> List[Dict[str, Any]]:
    ...
    with ThreadPoolExecutor(max_workers=min(5, len(sub_questions) or 1)) as executor:
        futures = {executor.submit(research_single_question, q, llm): q for q in sub_questions}
        for future in as_completed(futures):
            try:
                res = future.result()
                research_notes.append(res)
            except Exception as e:
                q = futures[future]
